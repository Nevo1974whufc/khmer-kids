@echo off
rem One-time: switch on automatic publishing. Windows. (macOS/Linux: setup_once.sh)
cd /d "%~dp0.."
echo === Mr Long ^& Ronnie: switch on auto-upload (one-time, ~2 min) ===
python -m pip install --user -q google-api-python-client google-auth-oauthlib
